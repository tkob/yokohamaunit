...
public class TableExtractVisitorTest {
    ...
    public void testExtractTables1() {
        List<Definition> definitions = Arrays.asList(
                new Table("table 1", Arrays.asList(new TableHeaderCell("a", yokohama.unit.ast.Span.dummySpan())), Arrays.asList(new Row(Arrays.asList(), yokohama.unit.ast.Span.dummySpan())), yokohama.unit.ast.Span.dummySpan()),
                new yokohama.unit.ast.Test("test name", Arrays.asList(), 0, yokohama.unit.ast.Span.dummySpan()),
                new Table("table 2", Arrays.asList(new TableHeaderCell("a", yokohama.unit.ast.Span.dummySpan())), Arrays.asList(new Row(Arrays.asList(), yokohama.unit.ast.Span.dummySpan())), yokohama.unit.ast.Span.dummySpan())
        );
        Group group = new Group(definitions, yokohama.unit.ast.Span.dummySpan());
        TableExtractVisitor instance = new TableExtractVisitor();
